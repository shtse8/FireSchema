...
export async function generate(target: OutputTarget, schema: ParsedFirestoreSchema, config: FirestoreODMConfig): Promise<void> {
  ...
  const sdkOption = 'client'; // Hardcoded for this adapter
  ...
  if (target.package) {
    try {
        const packageJsonContent: any = {
            ...
            peerDependencies: { "@shtse8/fireschema-runtime": "^0.1.0", "firebase": "^9.0.0 || ^10.0.0 || ^11.0.0" },
            devDependencies: { "@types/jest": "^29.5.14", "firebase": "^10.12.4", "jest": "^29.7.0", "ts-jest": "^29.3.1", "typescript": "^5.0.0" }
        };
