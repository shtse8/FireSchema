...
/**
 * Generates TypeScript Admin ODM code.
 */
export async function generate(target: OutputTarget, schema: ParsedFirestoreSchema, config: FirestoreODMConfig): Promise<void> {
  ...
  if (!fs.existsSync(modelTemplatePath)) throw new Error(`TS Admin Adapter: Model template not found at: ${modelTemplatePath}`);
  if (!fs.existsSync(collectionRefTemplatePath)) throw new Error(`TS Admin Adapter: CollectionRef template not found at: ${collectionRefTemplatePath}`);
  if (!fs.existsSync(queryBuilderTemplatePath)) throw new Error(`TS Admin Adapter: QueryBuilder template not found at: ${queryBuilderTemplatePath}`);
  if (!fs.existsSync(updateBuilderTemplatePath)) throw new Error(`TS Admin Adapter: UpdateBuilder template not found at: ${updateBuilderTemplatePath}`);
